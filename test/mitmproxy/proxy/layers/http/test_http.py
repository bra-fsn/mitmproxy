...
def test_http_reply_from_proxy_streamed(tctx):
    """Test a streaming response generated by an addon using an iterable body."""

    def reply_streaming(flow: HTTPFlow):
        flow.response = Response.make(200, b"")
        flow.response.headers["content-length"] = "6"
        flow.response.stream = iter([b"abc", b"def"])
...
def test_http_reply_from_proxy_streamed_generator(tctx):
    """Test addon-generated response with a generator that yields multiple chunks."""

    def file_like_generator():
        yield b"Hello "
        yield b"World"
        yield b"!"
    ...
    def reply_streaming(flow: HTTPFlow):
        flow.response = Response.make(200, b"")
        flow.response.headers["content-length"] = "12"
        flow.response.stream = file_like_generator()
...
def test_http_reply_from_proxy_streamed_empty(tctx):
    """Test addon-generated streaming response with empty iterable (HEAD-like)."""

    def reply_streaming(flow: HTTPFlow):
        flow.response = Response.make(200, b"")
        flow.response.stream = iter([])
